...
then
  ...
  # turn off /mnt2
  sed -i 's/\/mnt\/spark,\/mnt2\/spark/\/vol0\/spark/g' ~/spark/conf/spark-env.sh
  # redirect log folder
  echo "export SPARK_WORKER_DIR=/vol0/spark/work" >> ~/spark/conf/spark-env.sh
  # mem size
  ...
  # other configs
  cat /mnt/demo-kit/$DATASET/conf/$DATASCALE/conf >> ~/spark/conf/spark-defaults.conf
